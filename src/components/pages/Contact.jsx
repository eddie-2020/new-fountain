...
const ContactUs = () => {
  ...
  const handleNetworkChange = () => {
    if (!navigator.onLine) {
      setAlert({ type: "network_error", message: "Network connection is offline." });
      setTimeout(() => {
        setAlert(null);
      }, 5000);
    } else {
    ...
    if (isValid) {
      ...
      emailjs
        ...
        .then((response) => {
          ...
          setEmail("");
          setMessage("");
          setAlert({ type: "successful", message: "Message sent successfully!" });
          setTimeout(() => {
            setAlert(null);
          }, 5000);
        })
        ...
        {alert && (
          <div
            className={`fixed top-0 right-0 mt-4 mr-4 z-50 bg-${
              alert.type === "network_error" ? "red" : "" || alert.type === "successful" ? "green" : ""
